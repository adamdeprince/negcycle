...
Avx512ArbitrageDetector::find_best_arbitrage(int max_cycle_length) {
  ...
  // AVX-512 wants contiguous float rows. Build dense weight matrix and its transpose.
  ...
  for (int start = 0; start < N; ++start) {
    ...
    for (int a : adj_start) {
      ...
      if (max_cycle_length >= 3) {
        ...
        for (; j + 16 <= N; j += 16) {
          const __m512 idx_v =
              _mm512_loadu_ps(index_f.data() + static_cast<std::size_t>(j));
...
NB_MODULE(_avx512, m) {
  arbcycle::bind_detector_module<arbcycle::Avx512ArbitrageDetector>(
      m,
