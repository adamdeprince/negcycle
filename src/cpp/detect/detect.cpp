...
bool supports_x86_avx2() noexcept {
    if (!supports_x86_avx()) {
        return false;
    }
    const auto leaf0 = cpuid(0, 0);
    ...
    constexpr std::uint32_t EBX_AVX2 = 1u << 5;
    return (leaf7.ebx & EBX_AVX2) != 0;
}
...
bool apple_arm_cap(int bit) noexcept {
    ...
    std::array<unsigned char, 16> caps{};
    size_t size = caps.size();
    if (sysctlbyname("hw.optional.arm.caps", caps.data(), &size, nullptr, 0) != 0) {
        return false;
    }

    const size_t byte = static_cast<size_t>(bit / 8);
    const unsigned offset = static_cast<unsigned>(bit % 8);
    if (byte >= size) {
        return false;
    }

    return ((caps[byte] >> offset) & 1u) != 0;
}
...
#if defined(STRIDE_ALIGN_HAVE_X86_AVX2)
            return supports_x86_avx2();
#else
            return false;
#endif
